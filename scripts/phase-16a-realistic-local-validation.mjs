/**
 * Phase 16A: restore the local 0023 dump in isolated Docker Postgres,
 * apply 0024–0030, import Batch 03, validate. Never touches production.
 */
...
const REPORT_JSON = join(SNAPSHOT_DIR, "2026-08-29-phase-16a-local.json");
const RESTORE_LOG = join(SNAPSHOT_DIR, "2026-08-29-phase-16a-restore.log");
...
async function main() {
  ...
  const name = `peptix-16a-${Date.now()}`;
...
`,
    ...
    console.log("PHASE_16A_LOCAL_RUN_COMPLETE");
  } finally {
    try {
