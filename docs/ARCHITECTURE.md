...
**Catalog:** `products`, `product_price_history`. One catalog, three shop areas (`shop`, `group_buy_1`, `group_buy_2`) in `shop_areas` + `shop_area_role_access` (migration `0051`). Customers never SELECT raw list prices; they use `list_shop_products` (retail Shop) or `list_shop_products_for_area`. Pipeline: catalog unit (`sell_unit_price` / oils pack conversion) → area formula (retail: kit/10×5 or unit×5; group_buy: existing unit) → existing `apply_role_markup` once. Config tables `retail_price_factor` / `kit_unit_divisor` (seeded 5 and 10). Roles remain `customer_roles` (1:1 `user_customer_roles`); a seeded **Group Buy** role is unassigned until an admin assigns it. Admin surcharge reporting still uses pre-markup catalog unit vs selling line at checkout.
