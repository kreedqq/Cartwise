...
**Catalog:** `products`, `product_price_history`. One catalog, three selling areas (`shop`, `group_buy_1`, `group_buy_2`) in `shop_areas` + `shop_area_role_access` (`0051`). `/shop` is retail Einzelverkauf; `/shop/group-buy-1` and `/shop/group-buy-2` are separate Kit-Gesuche instances of the existing Group-Buy system (not a second product table). Additive `0052` adds `shop_area_products` (hide SKU per area), `shop_area_product_prices`, `shop_area_product_role_markups`, `shop_area_documents`, and `kit_shares.shop_area`. Customers never SELECT raw list prices; they use `list_shop_products_for_area`. Pipeline: optional area catalog override → area formula (retail: kit/10×5 or unit×5; group_buy: existing kit/tier unit) → `markup_percent_for_area` (optional product×area×role, else `customer_roles.markup_percent`) → existing `apply_role_markup` once. Config tables `retail_price_factor` / `kit_unit_divisor` (seeded 5 and 10). Roles remain `customer_roles` (1:1 `user_customer_roles`); a seeded **Group Buy** role is unassigned until an admin assigns it. Admin surcharge reporting still uses pre-markup catalog unit vs selling line at checkout. Admin hub: `/admin/shop-areas` (Verkaufsbereiche).
