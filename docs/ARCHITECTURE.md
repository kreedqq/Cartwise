...
`AuthProvider` + `ProtectedRoute` / `UsernameGate` / `AdminRoute`. Discord and Telegram (`custom:telegram`) OAuth use `skipBrowserRedirect` and never assign GoTrue JSON to `window.location`. Password login does not navigate to `/dashboard` until AuthProvider has a session. After a session exists, `UsernameGate` sends users with a missing `profiles.username` or an admin Telegram-reauth flag (`username_required_on_next_login`, only after a fresh `SIGNED_IN`) to `/username-required` (outside `AppShell`). Reauth applies `preferred_username` from the linked `custom:telegram` identity via `apply_telegram_reauth_username` (no free-text change). OAuth callback (`completeOAuthCallback`) keeps a session if `exchangeCodeForSession` fails after `detectSessionInUrl` already consumed the code. Telegram login does not merge accounts by username. Anon key is public; authorization is RLS + RPCs. Production Site URL must be `https://peptix.app` (not localhost). Admin user delete is `admin_delete_user` (admin-only SECURITY DEFINER); it does not cascade-delete orders.
...
**Catalog:** `products` is the global SKU master only. Assortment per selling area (`shop`, `group_buy_1`, `group_buy_2`) is the applied dealer file (`shop_area_products` + `shop_area_product_prices` via `apply_area_vendor_catalog`, `0056`/`0057`/`0058`). Area display categories live in `shop_area_categories` plus `shop_area_products.imported_category_key` / `manual_category_key` (`0059`). Storefront grouping uses `list_shop_area_storefront`, never `products.category`. Area grundpreis is `coalesce(manual_price_usd, imported_price_usd)` written to `price_usd`. No global-active fallback and no category whitelist. Area documents live in `shop_area_documents` (the applied file only; upload happens first, then `apply_area_vendor_catalog` updates catalog + pointer in one DB transaction). Customers never SELECT raw list prices; they use `list_shop_products_for_area`. Pipeline: effective area grundpreis → area formula (retail: vendor÷10×factor; group_buy: vendor×factor) → global role markup once (`0053`/`0054`). Admin `/admin/shop-areas` is Händlerkatalog / Produkte / Preise. Global import routes exist but are off the normal admin nav.

**Cart:** `carts` (including `shop_area`, default `shop`), `cart_items`. One open cart is activated per area via `ensure_shop_area_cart`; mixed-area lines are not used. Kits never enter a retail cart (`kit_share_target_cart_id`, trigger). View `cart_summaries`.

**Orders:** `orders` (telegram username + Lieferart + address snapshots from checkout; `0042`/`0045`; `shop_area` snapshot from `0051`, NULL = legacy), `order_items`, `order_status_history`, `order_admin_notes`. Status workflow (`0047`): pending, processing, dispatched, received, shipped, completed; historical confirmed/cancelled remain valid. Visual delivery progress is `order_progress` (`0049`/`0050`): one row per order (`status_key`, `progress_percent`, `comment`, `title`); customers read only their own order; upserts go through admin-only `upsert_order_progress`. Admin Übersicht writes seven fixed customer-facing statuses from `SHIPPING_PROGRESS_STATUSES` and does not change `orders.status`. Shipment tracking columns on `orders` (`tracking_number`, `tracking_carrier`, `tracking_url`, assignment + notification timestamps) are written only via admin-only `upsert_order_tracking`. Shipping cost fields (China/DE) from 0020+ live on `/admin/shipping-costs`. Admin order management is `/admin/orders` (Übersicht) plus `/admin/orders/:id`; legacy `/admin/shipping` redirects there. Übersicht can filter by stored `shop_area`. `create_order` validates payment, Lieferart, matching address fields, and shop-area access server-side, then recomputes catalog prices via `shop_area_sell_unit_price`. Admin Bestell Zusammenfassung aggregates processing `order_items` by SKU. Quantity labels for shop, cart, orders, and PDF come from `src/lib/quantityFormat.ts` (Kit/Kits, Vial/Vials, Packung/Packungen; kit fractions only with a real `kit_share` identity; Shop retail peptides/water use Vial).
...
**Kit sharing (invite) + Kit Gesuche (marketplace):** both use `kit_shares` / `kit_share_participants`. Group Buy 1 and Group Buy 2 are two instances of this marketplace (`kit_shares.shop_area`, default `group_buy_1`). `/kit-gesuche` redirects to the first allowed Group-Buy area. Invite kits keep `is_open_request = false` (RLS: creator or participant). Direct SELECT policies use `user_participates_in_kit_share` (SECURITY DEFINER, same membership as before) so participant policies do not recurse on `kit_share_participants`; admin SELECT policies are unchanged (`0048`). Open marketplace kits (`is_open_request = true`) are listed via SECURITY DEFINER RPCs (`list_open_kit_requests`, `join_kit_request`, …) scoped by `_shop_area`. Join locks the kit row (`FOR UPDATE`) so remaining vials cannot be overbooked. Cart lines are created only when an open request becomes `full`, through the existing `kit_share_sync_*` helpers and `profiles.username` cart names, into that kit's area cart. The Kit Gesuche product filter is scoped by shop category so BPC never appears under Reconstitution Water. See `docs/KIT_REQUESTS.md`.
