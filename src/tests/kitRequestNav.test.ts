...
describe("Group Buy navigation and kit structure", () => {
  it("routes Group Buy 1 and 2 to GroupBuyPage (catalog + kits), /kit-gesuche redirects via KitRequestsPage", () => {
    ...
    expect(app).toContain('path="/shop/retail" element={<ShopRetailPage />}');
    expect(app).toContain('path="/shop/group-buy-1" element={<GroupBuyPage />}');
    expect(app).toContain('path="/shop/group-buy-2" element={<GroupBuyPage />}');
  ...
  it("GroupBuyPage contains kit request logic", () => {
    ...
    expect(read("src/pages/GroupBuy.tsx")).toMatch("isGroupBuyAreaKey");
