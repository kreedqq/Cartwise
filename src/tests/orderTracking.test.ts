...
describe("shipping SQL and cancel wiring", () => {
  it("stores tracking on orders through an admin RPC and never deletes cancelled orders", () => {
    ...
    expect(read("src/pages/admin/AdminShipmentManage.tsx")).toContain('status: "cancelled"');
    expect(read("src/pages/admin/AdminShipmentManage.tsx")).toContain("CancelOrderDialog");
    expect(read("src/pages/admin/AdminShipmentManage.tsx")).toContain("Bestellung stornieren");
    ...
    expect(read("src/App.tsx")).toContain("AdminShipmentCenterPage");
  });
});
