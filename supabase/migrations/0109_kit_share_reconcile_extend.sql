-- 0109_kit_share_reconcile_extend.sql
-- Read-only reconciliation report (replaces 0108 body). No data mutations.
...
begin
  ...
  loop
    _cart_id := public.kit_share_target_cart_id(_participant.user_id, _kit.shop_area);
    _cart_qty := 0;
    _line_count := 0;
    _line_area := null;
  ...
  return jsonb_build_object(
    ...
    'checkedAt', _checked_at,
    'overallStatus', case
      when jsonb_array_length(_issues) = 0 then 'HEALTHY'
      when _kit.status in ('cancelled', 'ordered') and not exists (
        select 1
