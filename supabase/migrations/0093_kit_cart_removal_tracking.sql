-- 0093_kit_cart_removal_tracking.sql
-- Track voluntary kit cart line removal on kit_share_participants; restore via RPC.

alter table public.kit_share_participants
  add column if not exists cart_line_removed_at timestamptz,
  add column if not exists cart_line_last_in_cart_at timestamptz;

comment on column public.kit_share_participants.cart_line_removed_at is
  'Set when the participant deletes their kit cart line (not kit sync cleanup).';
comment on column public.kit_share_participants.cart_line_last_in_cart_at is
  'Updated when kit_share_sync_participant_cart upserts their kit cart line.';

create or replace function public.track_kit_cart_item_removal()
returns trigger
language plpgsql
security definer
...
declare
  _user_id uuid;
begin
  if old.kit_share_id is null then
    return old;
  end if;
  if coalesce(current_setting('kit.skip_cart_removal_tracking', true), '') = 'on' then
    return old;
  end if;
  ...
  update public.kit_share_participants ksp
  set cart_line_removed_at = now(),
      updated_at = now()
  where ksp.kit_share_id = old.kit_share_id
    and ksp.user_id = _user_id
    and ksp.ordered_at is null
    and ksp.order_id is null;

  return old;
end;
$$;

drop trigger if exists cart_items_track_kit_removal on public.cart_items;
create trigger cart_items_track_kit_removal
  before delete on public.cart_items
  for each row
  execute function public.track_kit_cart_item_removal();
...
begin
  ...
  if exists (
    ...
    perform public.log_audit(
      _actor, 'kit_cart.restore', 'kit_share', _kit_share_id, _participant_user_id,
      jsonb_build_object('participantUserId', _participant_user_id, 'alreadyInCart', true)
    );
