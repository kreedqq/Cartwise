-- 0117_admin_delete_carts.sql
-- Admin bulk soft-delete of open customer carts (open cart_items removed; kit_shares untouched).
...
begin
  ...
  foreach _id in array _unique loop
    ...
    if _cart.status = 'ordered' then
      raise exception 'Bestellte Warenkörbe können nicht gelöscht werden.' using errcode = 'P0001';
    end if;
    if not exists (
      select 1
