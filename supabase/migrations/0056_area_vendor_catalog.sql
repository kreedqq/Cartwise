-- 0056_area_vendor_catalog.sql
-- Implements vendor-catalog-driven product visibility per shop area.
--
-- CHANGE OF SEMANTICS: product_visible_in_shop_area
--   BEFORE (0052): allowlist-absent = show all; explicit is_active=false hides.
--   AFTER  (0056): allowlist-required = only products with an explicit
--                  shop_area_products row (is_active=true) are visible.
--                  If no rows exist for an area the catalog is empty.
--
-- This means the three shop areas are now fully independent vendor catalogs.
-- A product that exists globally but has no shop_area_products row for an area
-- will NOT appear in that area's catalog.
--
-- IMPORTANT: existing production areas will have an empty catalog until a
-- vendor document is imported for them via apply_area_vendor_catalog().
--
-- ADDED:
--   apply_area_vendor_catalog(_area_key, _rows)  – replaces catalog atomically
--
-- UPDATED:
--   product_visible_in_shop_area                 – allowlist semantics
--   create_order                                  – FAIL CLOSED: reject entire order
--                                                  if any non-kit item is outside
--                                                  the area's vendor catalog allowlist
--
-- NO table/column/policy changes. shop_area_products was created in 0052.
-- Migration 0051–0055 are unchanged.

-- ---------------------------------------------------------------------------
-- product_visible_in_shop_area: allowlist mode
-- ---------------------------------------------------------------------------
...
as $$
  select exists (
    ...
    join public.products p on p.id = sap.product_id
    where sap.product_id  = _product_id
...
-- Called by the admin after parsing a vendor document client-side.
-- _rows: [{product_id uuid, price_usd numeric, bulk_price_usd numeric|null,
--          bulk_price_min_quantity numeric|null}]
-- ---------------------------------------------------------------------------
...
begin
  ...
  -- Count existing catalog entries before replacement
  select count(*)::integer into _prev_count
  from public.shop_area_products
  where shop_area_key = _area_key;

  -- Atomic catalog replacement: delete old, insert new
  ...
  loop
    ...
    -- Catalog membership
    insert into public.shop_area_products (shop_area_key, product_id, is_active, updated_at)
    values (_area_key, _row.product_id, true, now())
    on conflict (shop_area_key, product_id)
    do update set is_active = true, updated_at = now();

    -- Vendor price (replaces global price for this area)
...
revoke all on function public.apply_area_vendor_catalog(text, jsonb) from public;
grant execute on function public.apply_area_vendor_catalog(text, jsonb) to authenticated;

-- ---------------------------------------------------------------------------
-- create_order: skip non-visible (non-catalog) non-kit items
-- This is a full replacement of the 0055 version with one added guard:
--   "if not product_visible_in_shop_area → continue"
-- placed immediately after the product look-up for non-kit items.
-- All other logic (pricing, bulk snapshot, kit handling, delivery, etc.)
-- is identical to migration 0055.
-- ---------------------------------------------------------------------------
